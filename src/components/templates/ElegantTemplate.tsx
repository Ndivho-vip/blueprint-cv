    ...
    <div className="a4-page bg-card grid grid-cols-[65%_35%]">
      ...
      <div className="px-6 py-6 flex flex-col border-r border-border">
        ...
        <div className="mb-4">
          ...
          <h1 className="font-oswald text-[30px] font-bold text-foreground leading-none tracking-tight">
            {cvData.name}
          </h1>
          <div className="w-12 h-[2px] bg-primary mt-2" />
          <p className="text-[10px] text-muted-foreground mt-2 tracking-wide">{cvData.title}</p>
        </div>

        {/* Summary */}
        <div className="mb-4">
          <h2 className="font-oswald text-[11px] font-bold uppercase tracking-[0.15em] text-foreground mb-1.5">Profile</h2>
          <p className="text-[8.5px] text-muted-foreground leading-[1.6]">{cvData.summary}</p>
        </div>
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <div
                    ...
                    className={`h-1 flex-1 rounded-sm ${
                      i < Math.round(s.level / 20) ? "bg-primary" : "bg-border"
                    }`}
                  />
                ))}
              </div>
